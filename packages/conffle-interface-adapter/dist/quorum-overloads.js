...
const overrides = {
    ...
    "getTransaction": (web3) => {
        ...
        web3.cfx.getTransaction.method.outputFormatter = (tx) => {
            ...
            // @ts-ignore
            web3.cfx.getTransaction.method, tx);
            // Perhaps there is a better method of doing this,
            // but the raw hexstrings work for the time being
            result.gas = _oldGas;
            return result;
    ...
    "getTransactionReceipt": (web3) => {
        ...
        // @ts-ignore
        web3.cfx.getTransactionReceipt.method.outputFormatter = (receipt) => {
            const _oldGasUsed = receipt.gasUsed;
            receipt.gasUsed = "0x0";
            let result = _oldTransactionReceiptFormatter.call(
            // @ts-ignore
            web3.cfx.getTransactionReceipt.method, receipt);
            // Perhaps there is a better method of doing this,
            // but the raw hexstrings work for the time being
            result.gasUsed = _oldGasUsed;
            return result;
    ...
    "decodeParameters": (web3) => {
        ...
        // result method
        function Result() { }
        web3.cfx.abi.decodeParameters = (outputs, bytes) => {
            ...
            web3.cfx.abi.mapTypes(outputs), `0x${bytes.replace(/0x/i, "")}`);
