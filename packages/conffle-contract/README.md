...
```
$ npm install truffle-conflux-contract
```
...
First, set up a new web3 provider instance and initialize your contract, then `require("truffle-conflux-contract")`. The input to the `contract` function is a JSON blob defined by [truffle-conflux-contract-schema](https://github.com/trufflesuite/truffle-conflux-contract-schema). This JSON blob is structured in a way that can be passed to all truffle-related projects.
...
In your `head` element, include truffle-conflux-contract:

```
<script type="text/javascript" src="./dist/truffle-conflux-contract.min.js"></script>
```

Alternatively, you can use the non-minified versions for easier debugging.

With this usage, `truffle-conflux-contract` will be available via the `TruffleContract` object:

```
var MyContract = TruffleContract(...);
```

**Note**: Web3 and its dependencies are now bundled into truffle-conflux-contract
v4.0.2 or higher.

### Full Example

Let's use `truffle-conflux-contract` with an example contract from [Dapps For Beginners](https://dappsforbeginners.wordpress.com/tutorials/your-first-dapp/). In this case, the abstraction has been saved to a `.sol` file by [truffle-conflux-artifactor](https://github.com/trufflesuite/truffle/tree/master/packages/truffle-conflux-artifactor):

```javascript
// Require the package that was previosly saved by truffle-conflux-artifactor
...
// how truffle-conflux-contract allows for easy control flow.
...
MetaCoin.at(contract_address).then(function(instance) {
  ...
  // This code block will not be executed until truffle-conflux-contract has verified
  // the transaction has been processed and it is included in a mined block.
  // truffle-conflux-contract will error if the transaction hasn't been processed in 120 seconds.
...
Even more helpful, however is we *don't even need* to use `.call` when a function is marked as `view` or `pure`, because `truffle-conflux-contract` will automatically know that that function can only be interacted with via a call:
...
This package is the result of breaking up EtherPudding into multiple modules. Tests currently reside within [truffle-conflux-artifactor](https://github.com/trufflesuite/truffle/tree/master/packages/truffle-conflux-artifactor) but will soon move here.
